    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-[#f8f8f8] overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex space-x-2 sm:space-x-3 mt-3 sm:mt-0">
            <button
              ...
              className="flex-1 sm:flex-auto px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-gray-700 bg-white rounded-full shadow-sm hover:bg-gray-50 border border-gray-200 transition-all duration-200 hover:shadow-md flex items-center justify-center"
            >
              <ChatBubbleBottomCenterTextIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1" />
              Feedback
            </button>
            <button
              ...
              className="flex-1 sm:flex-auto px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-gray-700 bg-white rounded-full shadow-sm hover:bg-gray-50 border border-gray-200 transition-all duration-200 hover:shadow-md flex items-center justify-center"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1">
      ...
      <main className="flex-1 flex flex-col max-w-6xl mx-auto w-full px-4 sm:px-8 overflow-hidden">
        <div 
          ...
          className="overflow-y-auto flex-1 scroll-smooth px-1 sm:px-3 pt-4 sm:pt-6 pb-2"
        >
          <AnimatePresence initial={false}>
            {messages.map((message) => (
              <motion.div
                ...
                className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'} mb-4 sm:mb-7`}
              >
                {message.type === 'property' ? (
                  <div className="w-full max-w-lg">
                    <div className="flex items-start mb-3">
                      ...
                      <div className="chat-bubble chat-bubble-assistant rounded-[18px] py-3 sm:py-3.5 px-3.5 sm:px-4 bg-white shadow-[0_2px_15px_rgba(0,0,0,0.03)] text-gray-800">{message.content}</div>
                    </div>
                    <div className="ml-10 sm:ml-12 relative group">
                      <PropertyCard 
                  ...
                  <div className="flex items-start max-w-[85%] sm:max-w-[75%] space-x-2 sm:space-x-3">
                    {message.type === 'assistant' && (
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-gray-800 to-black flex-shrink-0 flex items-center justify-center text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 sm:w-5 sm:h-5">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 9.75a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 0 1 .778-.332 48.294 48.294 0 0 0 5.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" />
                        </svg>
                      </div>
                    ...
                    <div 
                      className={`chat-bubble rounded-[16px] sm:rounded-[18px] py-3 sm:py-3.5 px-3.5 sm:px-4 ${
                        message.type === 'user' 
                    ...
                    >
                      <div className={`prose ${message.type === 'user' ? 'prose-invert' : ''} max-w-none prose-sm prose-p:my-1 prose-headings:my-2 prose-ul:my-1 prose-li:my-0.5 prose-pre:my-1 prose-pre:bg-gray-100/10 prose-pre:rounded-lg prose-pre:p-2 text-sm sm:text-base`}>
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            pre: ({ children, ...props }: ComponentPropsWithoutRef<'pre'>) => (
                              <pre className="bg-gray-100/10 rounded-lg p-2 overflow-x-auto text-sm" {...props}>{children}</pre>
                            ),
                    ...
                    {message.type === 'user' && (
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full overflow-hidden flex-shrink-0 bg-gray-100">
            ...
            {isLoading && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex justify-start mb-4 sm:mb-7"
              >
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-gray-800 to-black flex-shrink-0 flex items-center justify-center text-white mr-2 sm:mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 sm:w-5 sm:h-5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 9.75a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 0 1 .778-.332 48.294 48.294 0 0 0 5.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" />
      ...
      <div className="sticky bottom-0 bg-white/80 backdrop-blur-xl py-3 sm:py-5 px-4 sm:px-8 border-t border-gray-100/40 flex-shrink-0">
        <form onSubmit={handleSubmit} className="max-w-6xl mx-auto">
          <div className="flex items-end rounded-[16px] sm:rounded-[18px] bg-white shadow-[0_2px_15px_rgba(0,0,0,0.03)] overflow-hidden transition-all duration-300 hover:shadow-[0_4px_20px_rgba(0,0,0,0.05)] focus-within:shadow-[0_4px_25px_rgba(0,0,0,0.08)]">
            <textarea
              ref={inputRef}
              value={input}
              ...
              placeholder="Message..."
              className="flex-1 py-3 sm:py-3.5 px-3.5 sm:px-4 resize-none min-h-[24px] max-h-[150px] border-none focus:outline-none bg-transparent text-gray-800 text-sm sm:text-base"
              rows={1}
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="p-3 sm:p-3.5 mr-1 disabled:text-gray-300 transition-colors duration-200"
              style={{ 
